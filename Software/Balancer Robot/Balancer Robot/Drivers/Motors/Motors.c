	...
	switch(motor)
	{
		case Motor_Left:
			if(value > 0)																						//PWM1 on, PWM2 off
			{
				...
				__HAL_TIM_SET_COMPARE(&PWM1_Timer, PWM1_CH, value);		//Set CCR PWM1 to value
			}
			else if(value < 0)																			//PWM2 on, PWM1 off
			{
				__HAL_TIM_SET_COMPARE(&PWM1_Timer, PWM1_CH, 0);				//Set CCR PWM1 to 0
				__HAL_TIM_SET_COMPARE(&PWM2_Timer, PWM2_CH, value * (-1));		//Set CCR PWM2 to value
			}
		...
		case Motor_Right:
			if(value > 0)																						//PWM1 on, PWM2 off
			{
				...
				__HAL_TIM_SET_COMPARE(&PWM3_Timer, PWM3_CH, value);		//Set CCR PWM3 to value
			}
			else if(value < 0)																			//PWM4 on, PWM3 off
			{
				__HAL_TIM_SET_COMPARE(&PWM3_Timer, PWM3_CH, 0);				//Set CCR PWM3 to 0
				__HAL_TIM_SET_COMPARE(&PWM4_Timer, PWM4_CH, value *(-1));		//Set CCR PWM4 to value
			}
			else																										//value = 0, turn off both PWM
			{
				__HAL_TIM_SET_COMPARE(&PWM4_Timer, PWM4_CH, 0);				//Set CCR PWM4 to 0
